...
export default function CourseDetailScreen() {
  ...
  const { classId, className, tab } = params;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [scheduleRes, materialRes, assignmentRes] = await Promise.all([
        ...
        fetch(`${API_URL}/api/student/course-content/${classId}/assignments`, { headers })
      ]);
  ...
  useEffect(() => {
    const nextTab = typeof tab === 'string' ? tab : Array.isArray(tab) ? tab[0] : '';
    if (nextTab === 'assignment' || nextTab === 'schedule' || nextTab === 'material') {
  ...
  const renderAssignment = ({ item }: { item: any }) => {
    ...
    const attemptAt = item.attempt_completed_at ? new Date(item.attempt_completed_at) : null;

    const isSubmitted = isQuiz ? !!item.attempt_id : !!item.submission_id;
    const completedAt = isQuiz ? attemptAt : submittedAt;
        ...
        onPress={() => {
          if (isQuiz) {
            router.push({
              ...
              params: { assignmentId: item.id, classId, className }
            });
          }
        }}
      >
