...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, SafeAreaView, StatusBar, ActivityIndicator, Alert, Platform } from 'react-native';
...
const StudentResultScreen = () => {
  ...
  const { attemptId, quizId, quizTitle, assignmentId, classId, className } = params;
  ...
  const fetchResultDetails = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/instructor/scores/attempt/${attemptId}`, {
  ...
  const handleDownload = async (format: 'pdf' | 'xlsx') => {
      ...
      try {
          ...
          const url = `${API_URL}/api/instructor/scores/attempt/${attemptId}/export?format=${format}`;
          ...
          const buildFallbackName = () => {
            ...
            return `${safeStudent}_${safeQuiz}_${ts}.${format}`;
          };
    ...
    if (router.canGoBack()) {
      ...
      if (quizId) {
        router.replace({
          ...
          params: { quizId, quizTitle, assignmentId, classId, className }
        });
      } else {
